...
export function CartDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      {/* Drawer */}
      <div className="fixed inset-y-0 right-0 w-full max-w-md bg-white dark:bg-neutral-900 shadow-2xl flex flex-col animate-slide-in-right">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b dark:border-neutral-700">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{t('cart.title')}</h2>
            {restaurant && (
              <p className="text-sm text-gray-500 dark:text-neutral-400">{restaurant.name}</p>
            )}
          </div>
          <button
            onClick={closeCart}
            className="w-10 h-10 rounded-full hover:bg-gray-100 dark:hover:bg-neutral-800 flex items-center justify-center transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto">
          {orderSuccess ? (
            <div className="flex flex-col items-center justify-center h-full p-6 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">{t('cart.orderPlaced')}</h3>
              <p className="text-gray-600 dark:text-neutral-400">{t('cart.orderPlacedDescription')}</p>
            </div>
          ) : items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full p-6 text-center">
              <svg className="w-16 h-16 text-gray-300 dark:text-neutral-600 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
              </svg>
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">{t('cart.empty')}</h3>
              <p className="text-gray-600 dark:text-neutral-400 mb-4">{t('cart.emptyDescription')}</p>
              <button
                onClick={closeCart}
        ...
        {items.length > 0 && !orderSuccess && (
          <div className="border-t dark:border-neutral-700 p-4 space-y-3 bg-gray-50 dark:bg-neutral-800">
            ...
            <button
              ...
              className="w-full py-3 bg-primary-500 hover:bg-primary-600 disabled:bg-gray-300 dark:disabled:bg-neutral-700 disabled:cursor-not-allowed text-white font-medium rounded-xl transition-colors flex items-center justify-center gap-2"
            >
              {isPlacingOrder ? (
                <>
            ...
            <button
              onClick={clearCart}
              className="w-full py-2 text-gray-600 dark:text-neutral-400 hover:text-red-600 text-sm transition-colors"
            >
              {t('cart.clearCart')}
            </button>
          </div>
